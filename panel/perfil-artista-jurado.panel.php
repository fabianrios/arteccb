...
<head>
	...
	$(function() {
		...
		$('.nav-anterior').click(function (evt) {
			...
			{
				$($('.fader').children('a')[($('.fader').children('a').length - 1)]).fadeIn();
			}
			e.fadeOut();
		});		
	});
							...
							foreach(range(1,3) as $number)
							{
								if(trim($user->__get('user_proyect_image_' . $number)) != '')
								{
									...
									if(file_exists($dir.$user->__get('user_proyect_image_' . $number)))
									{
										...
										?>
                                        <a href="<?php echo APPLICATION_URL.$dir.$user->__get('user_proyect_image_' . $number)?>" rel="lightbox" style="<?php echo $style?>" >
										<img src="<?php echo APPLICATION_URL.$dir.$user->__get('user_proyect_image_' . $number)?>" alt="" width="597" />								
										</a>
										<?php
									}
									else
							...
							<div class="six columns">
								...
								<div class="specs">
									<ul>
										...
										{
											if(trim($prize->__get('prize_name')) != '')
											{
												$country	= new Country($exposition->__get('country_id'));
												?>
												<li>
												...
												<li>
													<h6>Institución</h6>
													<span><?php echo $prize->__get('prize_institution')?></span>
												</li>
												<li class="city">
													<h6>Ciudad</h6>
													<span><?php echo $prize->__get('prize_city')?></span>
												</li>
												<li class="country">
													<h6>Pais</h6>
													<span><?php echo utf8_encode($country->__get('country_name'))?></span>
												...
												<li class="city">
													<h6>A&ntilde;o</h6>
													<span><?php echo $prize->__get('prize_year')?></span>
												</li>												
												<?php
											}
										}
